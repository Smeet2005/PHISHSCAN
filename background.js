  ...
  try {
    ...
    if (response.ok) {
      if (response.status === 204) {
        // Handle Rate Limit (204 No Content is common for VT rate limits)
        console.warn('VirusTotal API rate limit exceeded (204 No Content)');
        const rateLimited = {
          malicious: false,
      ...
      try {
        ...
        console.warn('VirusTotal API returned invalid JSON:', e);
        // Fallback for empty/invalid response
        const errorResult = {
          malicious: false,
          reason: 'VirusTotal: Empty or invalid response from server',
          error: 'INVALID_JSON'
        };
        vtCache.set(url, { timestamp: Date.now(), data: errorResult });
        return errorResult;
      }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  try {
    switch (request.type) {
      ...
      case 'CHECK_VIRUSTOTAL':
        ...
        checkVirusTotal(request.url).then(result => {
          try { sendResponse(result); } catch (e) { /* context invalidated */ }
        });
        return true; // Keep message channel open for async response
      ...
      case 'RESOLVE_SHORTENED_URL':
        ...
        resolveShortenedURL(request.url).then(resolvedUrl => {
          try { sendResponse({ resolvedUrl: resolvedUrl }); } catch (e) { /* context invalidated */ }
        });
        return true; // Keep message channel open for async response
    ...
    // Catch context invalidated errors or other message passing issues
    console.debug('Message handler error:', err);
  }
});
